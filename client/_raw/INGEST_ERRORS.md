...
then drop the files into `client/_raw/docs/inbox/` and re-run `/ingest`.
...
resulting files under `client/_raw/web/` and `client/_raw/assets/`.
...
saved to:
`client/_raw/web/www.thescla.org/index.md`
...
exports) into `client/_raw/docs/inbox/` and re-run `/ingest`. The
