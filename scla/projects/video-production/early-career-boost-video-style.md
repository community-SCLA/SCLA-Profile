...
How SCLA's lesson videos and scripts are built, reverse-engineered from the **Early Career Boost** track so we can generate HeyGen scripts directly from course material. Companion template: [`heygen-lesson-script.md`](heygen-lesson-script.md).
...
2. Run it through [`heygen-lesson-script.md`](heygen-lesson-script.md), which encodes the Style B structure and the voice rules above.
