---
source: https://www.thescla.org; Community Team Mondays notes Apr 2026 (archived externally)
generated_by: knowledge-curator
last_updated: 2026-05-13
---

# Glossary

> Every acronym, product code-name, and internal term found in available source material.

All definitions are derived from Google search-index snippets. Terms marked **TODO** could not be defined from available evidence.

---

## SCLA

- **Canonical form:** SCLA
- **Full name:** The Society for Collegiate Leadership & Achievement
- **Also seen as:** "The SCLA", "SCLA Honor Society", "SCLA – Accredited Collegiate Leadership Honor Society"

> "The Society for Collegiate Leadership & Achievement (SCLA), the leading College-to-Career Honor Society, empowers students to excel academically and professionally."

— *source: https://www.thescla.org & History ｜ SCLA's 4 Pillars & Growth (4_23_2026 3：38：37 PM).html*

Canonical identity statement and founding facts (dates, founder): [_archive/source-of-truth/charter.md](../../_archive/source-of-truth/charter.md) (archived provenance)

**Related:** [products-services.md](./products-services.md), [people.md](./people.md)

---

## College-to-Career Honor Society

- **Canonical form:** College-to-Career Honor Society
- **Also seen as:** "Campus to Career"

SCLA's self-applied category label. Distinguishes it from purely academic honor societies by emphasizing career readiness as a core deliverable alongside academic recognition.

> "SCLA is the leading College-to-Career Honor Society that empowers students to excel academically and professionally..."

— *source: [https://www.thescla.org](https://www.thescla.org)*

---

## NAB

- **Canonical form:** NAB
- **Full name:** National Advisory Board
- **Apply page:** [thescla.org/nab-apply](https://www.thescla.org/nab-apply)

A selective, compensated advisory group of higher education professionals who provide strategic input on SCLA's programming. Members are drawn from academic, administrative, and student-facing campus roles.

> "The SCLA National Advisory Board is a selective group of higher education professionals who provide strategic input on SCLA's programming, ensuring it aligns with current trends and effectively supports student development across campuses."

**What members do (three areas):**

1. **Advisory Board Meetings** — two 60-minute virtual sessions per year (January & June)
2. **Programmatic Feedback** — live or async input on SCLA's leadership and career-readiness content
3. **Support SCLA Locally** — light-touch campus contact to facilitate student access

**Compensation:** $2,000 stipend, awarded at milestone achievements
**Time commitment:** ~3–4 hours per academic year
**Other benefits:** National recognition, CEUs, professional network access, leadership credential
**NAB contact:** Kristen Cooper, Vice President of University Relations (941-713-2584)

— *source: https://www.thescla.org ｜ Apply to Join Our National Advisory Board (4_23_2026 3：42：11 PM).html*

TODO: needs input — current number of seats/members on the board; who chairs it.

**Related:** [people.md](./people.md)

---

## ISPI

- **Canonical form:** ISPI
- **Full name:** Innovation Strengths Preference Indicator™
- **Also seen as:** "ISPI strengths assessment"

A structured strengths assessment integrated into SCLA's member experience. It is listed in SCLA's assessments library with 3 sections and 64 questions.

ISPI is designed to help students identify their natural behavioral tendencies and leadership patterns — not to label, but to equip. It focuses on *application* of self-awareness to career decisions and professional positioning.

**What ISPI measures (four dimensions):**

1. **Decision-Making Style** — analytical vs. intuitive vs. collaborative vs. fast-execution
2. **Communication Tendencies** — direct/concise, diplomatic/relational, detail-oriented, or vision-driven
3. **Leadership Orientation** — initiative-taker, stabilizer, persuader, or structure-builder
4. **Work Environment Preferences** — structured systems, fast-paced startups, collaborative teams, or independent roles

**How SCLA uses it:** Integrated into the broader leadership development experience. Members use results in resume strategy, interview framing, career path selection, and goal-setting. Described as differentiating from tools like Myers-Briggs by emphasizing application over labeling.

> "The ISPI strengths assessment is a structured development tool designed to help students identify their natural behavioral tendencies and leadership patterns… ISPI focuses on application. It's not about labeling you—it's about equipping you."

— *source: https://www.thescla.org Campus to Career： Understanding the ISPI Strengths Assessment (4_23_2026 3：49：16 PM).html*

TODO: needs input — is ISPI a proprietary SCLA tool or licensed from a third party?

---

## Chapter

- **Canonical form:** Chapter
- **Also seen as:** "SCLA Chapter", "on-campus chapter"

A campus-based unit of SCLA. Students at a partner institution who join SCLA participate through their institution's chapter.

> "Start an SCLA Chapter | Lead & Earn Experience" (page title)

— *source: [https://www.thescla.org](https://www.thescla.org)*

TODO: needs input — chapter governance structure, officer roles, and how chapters are chartered. Source page: https://www.thescla.org/start-a-chapter (not yet scraped).

**Related:** [products-services.md#online-membership](./products-services.md#online-membership)

---

## Online Membership

- **Canonical form:** Online Membership
- **Also seen as:** "online_membership" (URL slug variant)

A membership pathway for students who do not have an on-campus chapter. Provides the same stated honors and tools as on-campus chapters.

> "Students without an on-campus chapter can join through online membership. Same honors and opportunities as on-campus chapters."

— *source: [https://www.thescla.org](https://www.thescla.org)*

**Related:** [products-services.md#online-membership](./products-services.md#online-membership)

---

## Career Readiness Certification

- **Canonical form:** Career Readiness Certification
- **Also seen as:** "Accredited Career Readiness Certification", "SCLA Career Readiness Certification", "Professional Readiness Program"
- **XP value:** 315 XP upon completion

The first and foundational step in a member's SCLA program journey. It is a 5-section, self-paced online program (~4 hours, 22 lessons) that awards a certificate of completion, college credit (at no additional cost), and 315 XP.

**Five sections:**

1. **Welcome & Orientation** — Introduction to SCLA, the leadership journey, and your Chapter (includes True Colors assessment)
2. **Setting SMART Goals** — Set and track personal leadership goals
3. **Peer Connections** — Build meaningful relationships with like-minded peers
4. **Career Connections** — Streamline job searching and professional networking
5. **Celebration** — Course completion and certificate award

**Skills earned:** Career Advocacy, Interview Skills, Active Listening, Adaptation to Change, Emotional Intelligence, Intrinsic Motivation, Critical Thinking, Communication, Time Management, Resilience

**College credit:** Awarded upon completion at no additional cost; enhances academic journey and résumé.

**Accreditation:** SCLA is described as "an accredited organization." The specific accrediting body is not named in available source material.

> "Begin your journey with the Career Readiness Certification, an exclusive opportunity to build skills, gain recognition, and earn college credit—all included with your membership."

— *source: https://www.thescla.org Program ｜ Career Readiness & College Credit (4_23_2026 3：40：23 PM).html; https://www.thescla.org Career Readiness Certification - The SCLA (4_23_2026 4：12：58 PM).html*

TODO: needs input — which accrediting body certifies the program? How many college credit hours are awarded?

---

## AI Career Hub

- **Canonical form:** AI Career Hub
- **Also seen as:** "AI-powered Career Hub"

A member-facing platform feature providing resume building and guided career conversations.

> "AI-powered Career Hub with resume builder and guided career chats"

— *source: [https://www.thescla.org](https://www.thescla.org)*

TODO: needs input — is this a proprietary tool or a third-party integration? Where is it accessed (members.thescla.org, app.thescla.org)? Source: https://www.thescla.org/benefits (not yet scraped).

---

## CEO Speaker Series

- **Canonical form:** CEO Speaker Series
- **Also seen as:** "C-suite voices" (description in snippet)

A networking and professional development benefit giving members access to C-suite speakers.

> "CEO Speaker Series (C-suite voices)"

— *source: [https://www.thescla.org](https://www.thescla.org)*

TODO: needs input — how often does this run, who organizes it, and is it live or recorded?

---

## Partner Campus

- **Canonical form:** Partner Campus
- **Also seen as:** "partner colleges", "850+ campuses"

Colleges and universities that have a formal relationship with SCLA allowing them to host a chapter and invite eligible students.

> "SCLA identifies high-achieving students meeting GPA requirements and partners with colleges nationwide."

— *source: [https://www.thescla.org](https://www.thescla.org)*

TODO: needs input — what is the formal agreement between SCLA and a partner campus? Is there a cost to the institution?

---

## Administrator

- **Canonical form:** Administrator
- **Also seen as:** "campus administrator" (implied)

The institutional point of contact who manages SCLA's relationship with a partner campus. A dedicated FAQ page exists at https://www.thescla.org/administrator-faq (not yet scraped).

TODO: needs input — full role definition, responsibilities, and access to SCLA systems.

---

## The SCLA Difference

- **Canonical form:** The SCLA Difference
- **Page:** https://www.thescla.org/the-scla-difference

Internal positioning term for SCLA's value proposition relative to other honor societies. Full content not available.

TODO: needs input — what are the stated differentiators? Source page: https://www.thescla.org/the-scla-difference (not yet scraped).

---

## 4 Pillars

- **Canonical form:** 4 Pillars
- **Full name:** 4 Pillars of Career Success
- **Seen in:** Mission & History page (https://www.thescla.org/mission-history)

SCLA's holistic framework for student success, introduced at founding in 2014. The four pillars are:

1. **Credentials & Connections**
2. **Health & Wellness**
3. **Personal Finance Literacy**
4. **Work and Life Skills**

> "At the heart of its program are the 4 Pillars of Career Success: Credentials & Connections, Health & Wellness, Personal Finance Literacy, Work and Life Skills. This comprehensive framework ensures that students are equipped not only with academic achievements but also with the real-world skills and connections needed to thrive."

— *source: https://www.thescla.org & History ｜ SCLA's 4 Pillars & Growth (4_23_2026 3：38：37 PM).html*

---

## Internal Jargon (from April 2026 meeting notes)

*Source: Community Team Monday meeting notes.*

---

## Canvas (Canva tracker)

- **Canonical form:** canvas (lowercase when referring to the tool; "Canva canvas" for clarity)
- **What it is:** A Canva document used as a project tracker or reporting surface — NOT a whiteboard app or PM tool.

Two specific canvases exist:
- **Team Projects tracker** — single source of truth for all action items (built by Kierra)
- **Community weekly sync canvas** — weekly bullet updates for leadership reporting (built by Kierra)

**Do not confuse** "canvas" with Canva-the-design-tool or any external PM product.

---

## Tech Team

- **Canonical form:** the tech team
- **What it is:** A 3-person startup crew distinct from the community team. Includes Sean/Shawn. Handles platform engineering and MJML email coding. Described as having "zero documentation."
- **Do not confuse** with "the community team" (the 10 Monday-meeting attendees).

---

## Community Team

- **Canonical form:** the community team
- **What it is:** The 10-person team that attends Community Team Mondays. Led by Amy Westby. Distinct from the tech team and leadership team.

---

## Focus Modes

- **Canonical form:** Focus Modes
- **What it is:** A dashboard filter system that categorizes members by persona (e.g., "career transitioner," "newbie"). Aug 1, 2026 launch. Content owned by Anushka Gupta. Mocked in Figma.
- **Do not confuse** with meditation, productivity focus modes, or any third-party tool.

---

## The Grand Project

- **Canonical form:** none confirmed — informal label only
- **What it is:** Amy Westby's term (Apr 20) for the Member Journey umbrella project.
- **Preferred term:** Member Journey

---

## OG Tracker

- **Canonical form:** OG tracker
- **What it is:** Amy's personal Google Sheet used before the Team Projects tracker (Canva) was established. Still exists but the Canva tracker is now the team-wide standard.

---

## Flying the Plane While Building It

- **What it is:** Zeketra Grandy's phrase (Apr 20) critiquing an acquisition-first strategy that prioritizes member volume before onboarding/support infrastructure is ready.
- **Context:** Used when discussing Member Journey gaps and onboarding capacity.
