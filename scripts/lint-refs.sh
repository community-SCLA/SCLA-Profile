...
echo "[1/16] Backtick path references resolve"
...
# ── 2. Word budgets ──────────────────────────────────────────────────────────
echo "[2/16] Instruction word budgets"
check_budget() {
...
ORCH_FILES=(AGENTS.md projects/video-production/CLAUDE.md
  .claude/rules/video-production.md .claude/skills/render-lessons/SKILL.md)
BUILDER_FILES=(AGENTS.md projects/video-production/CLAUDE.md
  .claude/rules/video-production.md projects/video-production/contracts/builder.md)
ORCH_WORDS=$(wc -w "${ORCH_FILES[@]}" | awk 'END {print $1}')
BUILDER_WORDS=$(wc -w "${BUILDER_FILES[@]}" | awk 'END {print $1}')
if [ "$ORCH_WORDS" -gt 2500 ]; then
  warn "video orchestrator route is $ORCH_WORDS words (budget 2500)"
else
  ok "video orchestrator route: $ORCH_WORDS/2500 words"
fi
if [ "$BUILDER_WORDS" -gt 3000 ]; then
  warn "video builder route is $BUILDER_WORDS words (budget 3000)"
else
  ok "video builder route: $BUILDER_WORDS/3000 words"
fi

# ── 3. No stale decisions-log paths ──────────────────────────────────────────
echo "[3/16] No references to old decisions-log path"
...
echo "[4/16] No unfilled template placeholders"
...
echo "[5/16] Critical files present"
...
echo "[6/16] No stray legacy hex values outside flagged locations"
...
echo "[7/16] No '_archive/source-of-truth/' routing pointers in live KB"
...
echo "[8/16] No retired scla/ path references in live files"
...
echo "[9/16] config/endpoints.json valid (schema + no secret material)"
...
echo "[10/16] STD-35: no doc claims a mechanism that does not exist"
...
echo "[11/16] No workspace carries an AGENTS.md"
...
PREPARE_PURGES="$(grep -c 'rm -f .*AGENTS.md' "$(dirname "$0")/batch-prepare.sh" \
  2>/dev/null || true)"
...
echo "[12/16] render-qa test suite"
...
echo "[13/16] Lesson-script layout: the folder name is the stage name"
...
echo "[14/16] PIPELINE-STATUS.md matches a fresh regeneration"
...
echo "[15/16] Agent sources never route to human overview files"
SOURCE_HITS=""
while IFS= read -r source_file; do
  [ -f "$source_file" ] || continue
  HIT="$(grep -nE 'README([.]md)?' "$source_file" 2>/dev/null || true)"
  if [ -n "$HIT" ]; then
    SOURCE_HITS="${SOURCE_HITS}${source_file}:${HIT}"$'\n'
  fi
done < <(
  printf '%s\n' AGENTS.md CLAUDE.md
  find .claude -type f -name '*.md' -not -path '*/_archive/*' 2>/dev/null
  find projects/video-production -type f \( -name AGENTS.md -o -name CLAUDE.md \) \
    -not -path '*/_archive/*' 2>/dev/null
  find projects/video-production/contracts -type f -name '*.md' 2>/dev/null
)
if [ -n "$SOURCE_HITS" ]; then
  warn "agent source points at a human overview file:"$'\n'"$SOURCE_HITS"
else
  ok "agent source graph contains no human-overview routes"
fi

echo "[16/16] Compact build kit and shared-only build infrastructure"
KIT="$WS_DIR/_run/BUILD-KIT.md"
INFRA_FAIL=0
if [ -f "$KIT" ]; then
  KIT_WORDS="$(wc -w < "$KIT")"
  if [ "$KIT_WORDS" -gt 3000 ] || grep -q 'BUILD-KIT:BEGIN\|BUILD-KIT:END' "$KIT"; then
    warn "generated build kit exceeds budget or contains extraction markers"
    INFRA_FAIL=1
  fi
fi
while IFS= read -r marker; do
  workspace="$(dirname "$marker")"
  GENERATORS="$(find "$workspace" -maxdepth 1 -type f -name 'make_*.py' 2>/dev/null || true)"
  if [ -n "$GENERATORS" ]; then
    warn "v2 workspace contains a bespoke generator:"$'\n'"$GENERATORS"
    INFRA_FAIL=1
  fi
done < <(find "$WS_DIR" -mindepth 2 -maxdepth 2 -name '.scla-control-v2' 2>/dev/null)
[ "$INFRA_FAIL" -eq 0 ] && ok "build kit is compact; v2 workspaces use shared infrastructure"
