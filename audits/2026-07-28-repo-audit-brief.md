...
| 7 | P1 — `.claude/rules/` replaces prose governance | ☑ **done** 2026-07-28 — both rules files created, every rule labelled with its mechanism (lint-refs+CI, .gitignore shield, PostToolUse hook) or honestly marked *Convention*; root CLAUDE.md = boot + routing + rules pointer + tool discipline (304/600 words); video CLAUDE.md's Critical Rules trimmed to a pointer + the operational details. Fresh-session rule loading not provable from inside this session — spot-check on next session |
| 8 | P3, R11 — retire dead governance machinery | ☑ **done** 2026-07-28 (R11 gate cleared by owner this session) — governance-check.sh deleted; skill-eval.sh + skill-eval.js + skill-rules.json retired as the coordinated change (lint-refs check 9 removed + renumbered, merge-settings.py registration dropped); linter still healthy |
| 9 | R4, R6 — import pattern; align the lesson | ☑ **done** 2026-07-28 (gate cleared by owner this session) — AGENTS.md is canonical, CLAUDE.md is 6 lines + `@AGENTS.md` import; frame.md:248 pointer repointed. career-transitions m2 aligned to mid-career-momentum's taxonomy (Redirect→Rebuild for forced-adjacent, old "Rebuild"→Forced Reinvention for forced-dramatic; labels verified against the canonical script) |
| 10 | R5/A2 — canonise `preview.sh`, delete `review.py` | ☑ **done** 2026-07-28 — preview.sh now starts the server, waits up to 30s for the port to actually answer, prints the forwarded https:// URL only for a live server, and fails loudly otherwise (both paths smoke-tested live: FAILED message on dead server; URL + HTTP 200 on real one). review.py deleted (referenced by nothing — verified). review.sh kept on the VS Code task's account |
| 11 | R7 — reshape `render-qa/` | ☑ **done** 2026-07-28 (gate cleared by owner this session) — **code + tests/ stayed flat on purpose** (positional `parents[3]` math and ~30 inbound refs untouched; README now warns against nesting them); `snag-log.md` stays at its canonical path (the un-editable settings hook hardcodes it) but rotated 501 KB → `logs/snag-log-archive-001.md`, keeping header + newest 5 entries (18 KB) + a written rotation policy; `BUILD-LOG.md` → `logs/`, HANDOFF doc → `docs/`. Verified: py_compile all tools, test suites 36/36 + 28/28 (needed `apt-get install ffmpeg` — env gap, not a regression), linter healthy. Full build not runnable (no workspace in flight; TTS/egress wall stands) — first real build re-verifies |
...
STEP 13 P4 ─ unwind .agents/ ─────────────────────────────────── ~1 h ──── LAST, on purpose
   ...
   │  · repoint render-qa/src/synth_narration.py and design-system/frame.md
