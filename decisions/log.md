...
## 2026-08-03 — Two stuck lessons resolved: duplicate m4 dropped, entrepreneur m2 routed to refinement
**Decision:** (1) `mid-career-momentum/m4_visibility-actions-what-they-are-and-how-to-practice-them` (blocked on owner since 2026-07-22) is **dropped from the program** — script and status record deleted, git history the archive. (2) `entrepreneur-accelerator/m2_why-build-your-own-path.txt` (parked in `unrefined/`, absent from the board) is **refined, not dropped**: cues stripped to plain spoken narration per `PROCESS.md`, moved to `refined/`, and given a scripted-stage status record. Both changes are executed by the round-3 Ringer run (`swarm-round3.json`, task `unblock-two-lessons`) as a reviewed patch, not committed directly.
**Rationale:** The blocked m4 file's body is the `m4_who-will-walk-this-next-chapter-with-you` narration sentence-for-sentence (pre-polish, with cues) — a mistitled duplicate — and its title topic is fully covered by the real `refined/m4_visibility-actions.txt`, which defines visibility actions and has learners design and test them. Rewriting would mean fabricating new curriculum; dropping loses nothing. The entrepreneur m2, by contrast, is real narration whose only defect is embedded `[On screen:]` shot directions — exactly what refinement exists for; without it the entrepreneur-accelerator track has no module-2 script and no status record. Owner delegated both calls 2026-08-03 ("use judgement to unblock").
**Owner:** community@thescla.org (decision delegated; judgment executed by Claude, Fable 5)
**Source:** Working session 2026-08-03; `~/.ringer` round-2 repo-org report (duplicate + cue findings re-verified against the live scripts this session)

## 2026-08-03 — Studio rebuild: the lone-builder pilot design is replaced by a staged Ringer studio with adversarial gates
**Decision:** The rendering pipeline is rebuilt (owner directive 2026-08-03: "forget everything that was in the pipeline beforehand... build me a new way of rendering high-quality videos using HyperFrames and Ringer"). (1) **PROCESS.md moves from six stages to eight:** `storyboarded` (design on paper, before any code, exit-gated by `checks/verify-storyboard.py` plus an independent critic panel's recorded verdict) and `certified` (exit-gated by `checks/verify-certification.py`: four pixel-audit lanes — presence, layout, timing, fidelity — PASS on extracted frames of the actual MP4, plus a fixed 3-persona SCLA-member screening panel majority, zero open P0/P1). Publish now requires `certified` + owner sign-off. (2) **No worker certifies its own output:** authors draft but never approve (self-approval fails the draft gate), builders implement approved storyboards but never certify, findings are fixed by workers who didn't find them, and the panel-gated stages are advanced only by the orchestrator after re-running verifiers and reading exit codes. (3) **The roster is manifests, not prose:** `studio-r1-storyboard.json` (authors: narration + storyboard draft), `studio-r2-critics.json` (3 lenses: brand/design-system, motion/engagement, instructional clarity — all reviewing both storyboards), `studio-r3-build.json` (builders), `studio-r4-gauntlet.json` (4 audit lanes × 2 pilots + personas Jaylen/Maria/Devon), orchestrated per the workdir `STUDIO.md` runbook; `swarm-pilots.json` and `swarm-round3.json` are superseded.
**Rationale:** Every serious defect that ever shipped (blank scenes, fabricated copy, dead-air holds) passed the automated gates and was caught only by human/vision review — so the new pipeline makes vision review and consumer reaction *structural* (workers whose whole job is looking at pixels and watching as members) instead of hoping the builder notices. Storyboard-before-code moves design critique to where a fix is a text edit, not a re-render. Critics are parallel and independent with orchestrator synthesis (adversarial-review pattern) rather than conversational, because personas and reviewers bleed when they share context.
**Owner:** community@thescla.org (executed by Claude, Fable 5)
**Source:** Working session 2026-08-03; Ringer template library (review-swarm, adversarial-review, focus-group, fix-swarm patterns)
