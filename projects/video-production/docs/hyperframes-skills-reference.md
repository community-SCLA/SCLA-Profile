...
Local skill pack for authoring HyperFrames video compositions (HTML → rendered MP4/WebM/MOV), originally installed via `npx skills add heygen-com/hyperframes`. Lives at `.claude/skills/` in the repo root (materialized there from the old `.agents/` store 2026-07-28, P4 — a future `npx skills add` re-install lands in `.agents/skills/`; re-materialize if you run it). This is a **separate tool from HeyGen** — HyperFrames authors standalone HTML compositions you can edit/diff/commit locally; HeyGen (web UI only — the Python batch-render path, `avatar-pipeline/`, was removed 2026-08-02) renders finalized `.txt` scripts as talking-head video. See `CLAUDE.md`'s Tool Routing section for how Synthesia/HeyGen web UI/HyperFrames relate.
...
| 3 · Support | **hyperframes-creative** | Non-animation creative direction: design specs (design-contract.md/design.md), palettes, typography, narration, beat planning, brand/style calls. | Deciding look-and-feel, narration, or creative direction. | Motion mechanics (→ hyperframes-animation). |
