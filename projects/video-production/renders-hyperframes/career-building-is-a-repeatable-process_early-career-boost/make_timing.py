...
GAP = 0.60       # s between one clip ending and the next starting
PRE = 0.20       # s a beat's picture leads its own audio
FINAL_HOLD = 1.80  # s of real audio after the last spoken word (rules file)
# GAP - PRE = 0.40s is the air every beat's picture holds after its own last
# word — above check_boundaries' MIN_AIR (0.2) and MIN_QUESTION_AIR (0.35).
#
...
def main():
    ...
    missing = [i for i in order if i not in voices]
    if missing:
        print(f"FATAL: no synthesized clip for {', '.join(missing)}", file=sys.stderr)
        return 2
    ...
    meta["total_duration_s"] = round(
        sum(float(voices[i]["duration_s"]) for i in order), 3)
    ...
    for bid in order:
        ...
        rows.append({"id": bid, "audio_start": round(t, 3),
                     "audio_dur": round(float(v["duration_s"]), 3)})
        t += float(v["duration_s"]) + GAP

    # 3. the picture leads its own audio by PRE; beat windows tile
    total = round(rows[-1]["audio_start"] + float(voices[order[-1]]["duration_s"])
                  + VIDEO_HOLD, 3)
    ...
    for r in rows:
        ...
        mid = r["vis_start"] + r["vis_dur"] / 2
        print(f"{r['id']}  vis {r['vis_start']:7.3f} +{r['vis_dur']:6.3f}   "
              f"mid {mid:7.2f}  audio {r['audio_start']:7.3f} "
              f"+{r['audio_dur']:6.3f}   air {air:5.2f}s")
