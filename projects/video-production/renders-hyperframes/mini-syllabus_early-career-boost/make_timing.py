...
# 0.65, not the reference build's 0.55: three of this lesson's beats end on a
# question ("...today?", "...world?", "...opportunities?"), and
# check_boundaries `question-clipped` wants >= 0.35s of air after a question
# mark. The visual cut lands at audio_start + dur + (GAP - PRE), so GAP - PRE
# is the floor on that air — 0.30 at the reference's numbers, which sits under
# the question rule with no margin. 0.65 puts the floor at 0.40.
GAP = 0.65
