...
| 2 | 00:02.5 | 00:08.0 | Transition at 2.48 ("easy"): the navy field pushes up and off frame as the paper ground rises beneath it. Light canvas. The line "it is easy to think a career begins with a title:" sets left in heading weight, entering in two parts — "it is easy to think a career begins with" at 3.52 ("career"), "a title:" at 5.16 ("title:") — then the first two pill chips fan to its right, each on its own word onset: intern 6.18, analyst 7.16. Chip copy is label-role type. Chips keep a clear column of air from the text block; no chip crosses it. | it is easy to think a career begins with a title: / intern / analyst | paper, ink, blue, chip, heading, label, entrance |
...
- **On-screen copy is the spoken key phrase, never the transcript.** Beats 12 and 22 carry only the contrast and the promise the narration lands on; beat 4 carries the governing lead-in "What they do not tell the whole story of:" above its three fragments so a muted viewer reads the thesis rather than its inverse; beat 8 keeps the script's own hedges, "often" and "formal". Beat 23's closing line is this script's closing line — nothing about a next-module subject is invented on screen.
...
### Studio round 5 — round-2 QA, run `scla-lesson-videos-20260803T190528Z`

Round-2 QA confirmed palette, type floors, timing (every beat inside ±0.053s),
copy fidelity and geometry, and left six localized defects. This is the fix
list; nothing else in the board changed, and the Panel Verdict is untouched.

- **Beats 2 and 18 — the outgoing beat's content rides the incoming ground.**
  Both cuts still flashed 0.30s of empty content region. The cause was not the
  ground transitions, which round 4 had already lengthened: the frames gate
  measures ink against the content region's OWN median colour, so a full-bleed
  navy frame at 2.50 and a full-bleed blue frame at 110.80 grade exactly as
  blank as a white one, and each stays uniform for the ~0.28s before the
  incoming ground climbs above y=960. Beat 1's lockup is now carried inside
  `#b2-navy` and pushes off frame with it; beat 17's badge, baseline and line
  hand over at the positions they held at 110.80 and settle down through the
  frame while the paper rises behind them — the board's own beat-18 transition,
  which the build had never staged. Each leaves as ONE tween target
  (`motion.exit-group`) and holds until its successor's copy has landed. Both
  are handoff copies of already-declared content, so every node is
  `data-layout-ignore`. Measured on the re-render: no blank content run at any
  point in the runtime.
- **Beat 16 — each caption takes its own half-width column.** The two opposed
  captions sat 34px apart on one shared baseline and read as a single run-on
  sentence, which left the gold (fills) and blue (drains) bars unlabelled. Each
  caption is now 380px inside its 476px column, so both wrap onto two lines of
  their own and bind to the bar directly beneath them. The columns sit 500px
  apart — the exact pitch of the two bar centres, x=60 and x=560 — and the
  measured clear air between the captions is 202px against the 120px floor. The
  caption box grows 116 → 170px to hold the wrap; the panel measures 554px
  inside its 560px box and the bars keep their geometry.
- **Beat 4 — the lead-in restores the script's qualifier.** "What they do not
  tell you:" said the opposite of the script, which is that titles do not tell
  the WHOLE STORY OF how you work, what you care about, where you create value.
  The three fragments and their 15.74 / 16.74 / 18.12 onsets are unchanged. The
  copy measures 795px on one line at 44px, and the box goes 1000 → 820px so the
  typewriter reveal lands with the last glyph instead of wiping empty paper
  after it; at 66px tall against a 63px glyph line the `overflow:hidden` clip
  still clears the cap-heights.
- **Beat 12 — the affirmed card carries the script's hedge, and the pair
  re-splits 700 / 900.** "Rather: seems to fit me right now." measures 790px at
  the 52px tight-quote scale, which set on one line inside the old 800px card
  but ate all 40px of its padding. Card 2 goes to 900px (content box 814px,
  24px clear of the copy) and card 1 — whose line is 455px — drops to 700px.
  The 80px gutter between them is unchanged, the gutter rule moves 836 → 736 to
  stay centred in it, the strike rule narrows 720 → 620 to keep its inset on
  the narrower card, and card 2 still ends on the 1680px content edge. This
  supersedes the "each 760px" figure in the beat-12 row and the Design Keys.
- **Beat 2 — the heading restores the script's framing.** "it is easy to think
  a career begins with a title:" is what makes beat 4's "But titles are only
  labels." a reversal rather than a contradiction. The longer first line
  measures 1130px in the full-width 1680px heading box and still sets on one
  line; the 3.52 / 5.16 entrances are unchanged. Beat 3 holds the same heading
  as static copy and carries the same edit, so the line does not change across
  the 8.0s cut.
- **Beat 13 — the stat numeral lands at 80.4, not 82.08.** The beat held the
  label and its rule alone at 0.65% content ink for 2.2s. The numeral is the
  beat, so it now arrives with the beat. This is the one place in the video
  where a visual precedes the word that names it ("three", 82.08); the owner's
  defect list is explicit that the near-bare frame is the defect being traded
  against, and the count-up settles on the numeral the narration then speaks.
- **Beat 14 — the panel slides in empty with the rail draw at 83.20.** Waiting
  for the copy's own cue opened the beat on a bare rail line for 0.94s. The
