...
Copy **only the spoken narration** from section 3 into a plain `.txt` saved directly in `lesson-scripts/<program-slug>/avatar/` — drop every cue, heading, and the shot list (those guide the human building visuals, not the avatar). Then render it manually via the HeyGen web UI (the batch/resumable code path, `avatar-pipeline/`, was removed 2026-08-02).
