...
- **`scla-stat` optional `icon`/`iconCue` (added 2026-07-29):** a small hero
  in the navy panel's otherwise-empty lower band, below the stat label —
  white main stroke, gold accents, same draw-on-cue contract as every other
  icon host. Added so a long stat sentence (narration well past the
  count-up/context beats) has a genuine mid-scene cued event to reach for
  instead of dead air; empty by default, existing builds compile unchanged.
