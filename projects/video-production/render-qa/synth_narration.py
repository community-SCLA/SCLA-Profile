...
What it does:
  ...
  3. Synthesizes one clip per scene (npx hyperframes tts), cached by a hash of
     (text, voice, speed) — only changed scenes re-synthesize.
  4. Trims each clip's lead/tail silence (peak-based, guarded, faded).
  5. Concatenates with real gaps: air after the scene's last sentence (0.3s,
     0.45s after a question) + 0.15s lead before the next scene.
  6. Writes assets/voice/narration.wav + assets/voice/scene-times.json — the
     ...
     boundaries. Deletes any stale transcript.json / *.pre-pad.* so a forgotten
     re-transcribe fails loudly downstream instead of silently misaligning.

After this, run:  npx hyperframes transcribe assets/voice/narration.wav
(cue anchors + the script gate still read Whisper), then compile_timeline.

Usage:
    synth_narration.py <workspace> [--voice af_heart] [--speed 0.95]
...
def tts(text: str, out: Path, voice: str, speed: str, cwd: Path):
...
def trim_clip(data, rate, chan):
    ...
    the cut edges. Returns a new array. Never trims into voiced audio."""
    n = len(data) // chan
    ...
    while first < n and window_peak(first) < TRIM_THRESHOLD:
        first += win
    if first >= n:
        return data[:]  # all silence — leave untouched, caller will notice
    last = n
...
def main():
    ...
    for i, (sc, text) in enumerate(zip(scenes, texts), start=1):
        ...
        sha = hashlib.sha1(f"{voice}|{speed}|{text}".encode()).hexdigest()[:16]
        if force or not clip.is_file() or old.get(clip_rel) != sha:
            tts(text, clip, voice, speed, cwd=ws)
    ...
    # Trim + concatenate with real gaps.
    out = array("h")
    rate = chan = None
    cursor = 0.0
    for i, (e, sc) in enumerate(zip(entries, scenes)):
        ...
        clip_data = trim_clip(data, rate, chan)
    ...
    manifest = {"mode": "per-scene", "voice": voice, "speed": speed,
                "rate": rate, "audio_end": audio_end,
                "air": AIR, "air_question": AIR_QUESTION, "lead": LEAD,
                "final_hold": FINAL_HOLD, "scenes": entries}
    manifest_path.write_text(json.dumps(manifest, indent=2))

    # Stale-artifact hygiene: the old transcript/padding belong to the old
    # audio. Deleting them makes a forgotten re-transcribe fail loudly.
    removed = []
    for stale in ("transcript.json", "transcript.pre-pad.json",
                  "narration.pre-pad.wav"):
        p = voice_dir / stale
        if p.exists():
            p.unlink()
    ...
    if as_json:
        print(json.dumps({"verdict": "OK", "audio_end": audio_end,
                          "scenes": len(entries), "removed": removed}, indent=2))
    else:
        print(f"[synth_narration] OK — {len(entries)} scenes, "
