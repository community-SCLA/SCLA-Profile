...
4. Create an Infisical Universal Auth machine identity restricted to reading this folder. Store its Client ID and Client Secret as GitHub Actions repository secrets **INFISICAL_CLIENT_ID** and **INFISICAL_SECRET**. These long-lived credentials are the explicitly approved simpler setup; rotate them if access changes. The unused **INFISICAL_MACHINE_ID** secret is not part of this workflow.
5. In GitHub, restrict the **community-email** environment to main. The approved Infisical project slug **scla-projects-n-joy** and environment **dev** are non-secret workflow configuration. The environment must allow the authorized scheduled worker to run unattended.
