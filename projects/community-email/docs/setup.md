...
3. Keep the approved Notion credential at the Infisical project root under **NOTION_API_KEY**. The worker finds the uniquely named **Practice draft — do not send** page and **Email generation requests** data source through Notion, so no page identifiers are stored in GitHub or Infisical.
4. Store the approved Infisical Universal Auth Client ID and Client Secret as GitHub Actions repository secrets **INFISICAL_CLIENT_ID** and **INFISICAL_SECRET**. On August 31, 2026, Kierra explicitly approved loading every root-level secret available to this identity into each community-email runner. The unused **INFISICAL_MACHINE_ID** secret is not part of this workflow. Rotate the credentials if access changes.
5. The approved Infisical project slug **scla-projects-n-joy** and environment **dev** are non-secret workflow configuration. The scheduled worker is enabled in the workflow and may be paused by changing its enable value to false in a reviewed commit.
6. Click the test button and manually run **Community email** once; verify Ready and copy the complete same-page code. Also test an invalid draft, corrected retry and duplicate click. Then verify an actual scheduled run.

To pause, set the workflow's COMMUNITY_EMAIL_ENABLED value to false. No relay, webhook host, Zapier, Make, Codex subscription change or local computer is required. Infisical remains the existing secret store.
