...
- **A published video is recorded before the next one starts.** The Wistia URL lands in `refinement-log.md` and is committed as part of the same pass. This is what makes a batch resumable: a stem is done if and only if it has a URL, so an interrupted run never strands rendered-but-unpublished work. *(Mechanism: `scripts/batch-ship.sh`; read the remaining queue with `scripts/batch-status.sh`.)*
